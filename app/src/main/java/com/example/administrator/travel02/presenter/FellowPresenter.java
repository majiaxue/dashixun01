...
public class FellowPresenter extends BasePresenter<FellowView> {
    private FellowModel model;
    @Override
    protected void initModel() {
        model=new FellowModel();
        mModels.add(model);
    }
    public void getLinkData(String token,int page){
        model.getLinkData(token, page, new ResultCallBack<TravelBean>() {
            @Override
